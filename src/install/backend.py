...
def _install_docker_images():
    ...
    docker_process = subprocess.run('docker pull fkiecad/fact_extractor',
...
def _create_firmware_directory():
    ...
    data_dir_name = config.get(
        'data_storage', 'firmware_file_storage_directory')
    ...
    chown_process = subprocess.run(f'sudo chown {os.getuid()}:{os.getgid()} {data_dir_name}',
                                   shell=True, stdout=PIPE, stderr=STDOUT, universal_newlines=True)
...
def _install_plugins(distribution, skip_docker, only_docker=False):
    ...
    for install_script in installer_paths:
        ...
        plugin = importlib.import_module(
            f'plugins.{plugin_type}.{plugin_name}.install')

        plugin_installer = plugin.Installer(
            distribution, skip_docker=skip_docker)
...
def _install_yara():  # pylint: disable=too-complex
    ...
    try:
        latest_url = requests.get(
            'https://hub.fastgit.xyz/VirusTotal/yara/releases/latest').url
        latest_version = latest_url.split('/tag/')[1]
    except (AttributeError, KeyError):
        raise InstallationError('Could not find latest yara version') from None

    yara_process = subprocess.run(
        'yara --version', shell=True, stdout=PIPE, stderr=STDOUT, universal_newlines=True)
    if yara_process.returncode == 0 and yara_process.stdout.strip() == latest_version.strip('v'):
        logging.info(
            'Skipping yara installation: Already installed and up to date')
        return

    logging.info(f'Installing yara {latest_version}')
    archive = f'{latest_version}.zip'
    download_url = f'https://hub.fastgit.xyz/VirusTotal/yara/archive/refs/tags/{archive}'
    ...
    if wget_process.returncode != 0:
        raise InstallationError(
            f'Error on yara download.\n{wget_process.stdout}')
    unzip_process = subprocess.run(
        f'unzip {archive}', shell=True, stdout=PIPE, stderr=STDOUT, universal_newlines=True)
    Path(archive).unlink()
    if unzip_process.returncode != 0:
        raise InstallationError(
            f'Error on yara extraction.\n{unzip_process.stdout}')
    yara_folder = [p for p in Path(
        '.').iterdir() if p.name.startswith('yara-')][0]
